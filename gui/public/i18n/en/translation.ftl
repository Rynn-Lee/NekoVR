...
websocket-connection_lost-desc = It looks like the NekoVR server crashed. Check the logs and restart the program.
websocket-timedout = Could not connect to the server
websocket-timedout-desc = It looks like the NekoVR server crashed or timed out. Check the logs and restart the program.
websocket-error-close = Exit NekoVR
websocket-error-logs = Open the logs Folder

## Update notification
version_update-title = New version available: { $version }
version_update-description = Clicking "{ version_update-update }" will download the NekoVR installer for you.
...
tips-turn_on_tracker = Using official NekoVR trackers? Don't forget to <b><em>turn on your tracker</em></b> after connecting it to the PC!
tips-failed_webgl = Failed to initialize WebGL.

## Navbar
navbar-home = Home
navbar-trackers_assign = Trackers
navbar-mounting = Mounting
navbar-body_proportions = Proportions
navbar-connect_trackers = Wi-Fi
navbar-ai_drift = AI Drift
...
tracker-settings-forget-description = Removes the tracker from the NekoVR Server and prevents it from connecting until the server is restarted. The configuration of the tracker won't be lost.
...
settings-general-steamvr-trackers-hands-warning = <b>Warning:</b> Enabling the SteamVR hand trackers will disable inputs from real controllers.
    This should only be enabled if you are using NekoVR for hand tracking.
...
settings-interface-theme-dark-orange = Dark Orange
settings-interface-theme-dark-purple = Dark Purple
settings-interface-theme-dark-gray = Dark Gray
settings-interface-theme-light-clean = Pure White
settings-interface-theme-dark-orange-desc = Dark graphite with vibrant orange accents
settings-interface-theme-dark-purple-desc = Dark obsidian with neon purple accents
settings-interface-theme-dark-gray-desc = Monochrome dark-gray aesthetic
settings-interface-theme-light-clean-desc = Crisp and clean light theme
...
settings-general-interface-connected_trackers_warning-description = This option will show a pop-up every time you try exiting NekoVR while having one or more connected trackers. It reminds you to turn off your trackers when you are done to preserve battery life.
...
settings-general-interface-use_tray-description = Lets you close the window without closing the NekoVR Server so you can continue using it without having the GUI bother you.
...
settings-general-interface-discord_presence-description = Tells your Discord client that you are using NekoVR along with the number of IMU trackers you are using.
...
settings-interface-behavior-error_tracking-description_v2 =
    ...
    <b>We do not collect personal information</b> such as your IP address or wireless credentials. NekoVR values your privacy!

    To provide the best user experience, we collect anonymized error reports, performance metrics, and operating system information. This helps us detect bugs and issues with NekoVR. These metrics are collected via Sentry.io.
...
settings-osc-router-network-description =
    ...
    These can be the same as other ports used in the NekoVR server.
...
settings-osc-vmc-description =
    ...
    to send NekoVR's bone data and receive bone data from other apps.
...
settings-utils-advanced-open_data-description-v1 = Open NekoVR's config folder in file explorer, containing the configuration
settings-utils-advanced-open_data-label = Open folder
settings-utils-advanced-open_logs = Logs folder
settings-utils-advanced-open_logs-description = Open NekoVR's logs folder in file explorer, containing the logs of the app
...
onboarding-home = Welcome to NekoVR
onboarding-home-description = Next-generation full-body motion tracking system
onboarding-home-start = Let's get set up!
onboarding-skip = Skip setup
onboarding-feature-quick = Quick Setup
onboarding-feature-precise = Precise Tracking
onboarding-feature-freedom = Wireless Freedom
...
onboarding-setup_warning =
    ...
    it is needed if this is your first time using NekoVR.
...
onboarding-quiz-usage-description = If you plan on using NekoVR for multiple purposes, you can change the affected settings later.
...
onboarding-quiz-mocap_preferences-desc = Specify how you plan to use NekoVR for mocap or VTubing
onboarding-quiz-mocap_preferences-playspace-title = What is your playspace?
onboarding-quiz-mocap_preferences-playspace-desc = If standing, NekoVR will try to track walking movement instead of anchoring you in one spot.
...
onboarding-wifi_creds-description-v2 =
    Most trackers (such as official NekoVR trackers) use Wi-Fi to connect to the server.
...
onboarding-connect_serial-error-modal-no_serial_device_found-desc =
    ...
    If this does not work:
      ...
      - try reinstalling the NekoVR server and select "USB Drivers" in the components section
...
onboarding-automatic_mounting-description = For NekoVR trackers to work, we need to assign a mounting orientation to your trackers to align them with your physical tracker mounting.
...
onboarding-automatic_proportions-description = For NekoVR trackers to work, we need to know the length of your bones. This short calibration will measure it for you.
...
onboarding-automatic_proportions-requirements-descriptionv2 =
    ...
    Your trackers and headset are connected to the NekoVR server and are working properly (ex. no stuttering, disconnecting, etc).
    Your headset is reporting positional data to the NekoVR server (this generally means having SteamVR running and connected to NekoVR using NekoVR's SteamVR driver).
...
onboarding-user_height-description = We need your height to calculate your body proportions and accurately represent your movements. You can either let NekoVR calculate it, or input your height manually.
...
trackers_still_on-modal-description =
    ...
    Do you still want to exit NekoVR?
trackers_still_on-modal-confirm = Exit NekoVR
...
status_system-StatusSteamVRDisconnected = { $type ->
    *[steamvr] Currently not connected to SteamVR via the NekoVR driver.
    [steamvr_feeder] Currently not connected to the NekoVR Feeder App.
}
...
status_system-StatusPublicNetwork = {$count ->
    [one] Your network profile is currently set to Public ({$adapters}). This is not recommended for NekoVR to function properly. <PublicFixLink>See how to fix it here.</PublicFixLink>
    *[many] Some of your network adapters are set to public: {$adapters}. This is not recommended for NekoVR to function properly. <PublicFixLink>See how to fix it here.</PublicFixLink>
}
...
firmware_tool-flashbtn_step-board_SLIMEVR = Turn off the tracker, remove the case (if any), connect the USB cable to your computer, then follow the appropriate steps for your NekoVR board revision:
...
firmware_update-devices-description = Please select the trackers you want to update to the latest version of NekoVR firmware.
...
unknown_device-modal-description = There is a new tracker with MAC address <b>{$deviceId}</b>.
    Do you want to connect it to NekoVR?
...
vrc_config-page-desc = This page shows the state of your VRChat settings and shows what settings are incompatible with NekoVR. It is highly recommended that you fix any warnings showing up here for the best user experience with NekoVR.
...
tracking_checklist-status-incomplete = You are not prepared to use NekoVR!
tracking_checklist-status-partial = {$count ->
    [one] You have 1 warning!
    *[many] You have {$count} warnings!
}
tracking_checklist-status-complete = You are prepared to use NekoVR!
...
tracking_checklist-STANDABLE_INSTALLED-desc =
    Standable frequently causes tracking issues when used alongside NekoVR. Standable should be fully uninstalled in Steam to ensure no issues arise.
...
tracking_checklist-NETWORK_PROFILE_PUBLIC-desc = {$count ->
    [one] Your network profile is currently set to Public ({$adapters}).
          This is not recommended for NekoVR to function properly.
...
dataset_recorder-title = Motion Dataset Recording
dataset_recorder-description = Record authoritative 50 Hz motion telemetry for AI drift training
dataset_recorder-status-idle = Idle
dataset_recorder-status-starting = Starting recorder...
dataset_recorder-status-recoverable = Recovery required
dataset_recorder-status-recording = Recording (50 Hz)
dataset_recorder-status-finalizing = Finalizing archive...
dataset_recorder-status-completed = Completed
dataset_recorder-status-cancelled = Cancelled
dataset_recorder-status-failed = Recording Failed
dataset_recorder-status-quarantined = Quarantined
dataset_recorder-action-start = Start Recording
dataset_recorder-action-stop = Stop Recording
dataset_recorder-action-stopping = Stopping...
dataset_recorder-action-cancel = Cancel
dataset_recorder-action-cancelling = Cancelling...
dataset_recorder-action-confirm_cancel = Are you sure you want to cancel recording? Partial data will be discarded.
dataset_recorder-action-export = Export Archive
dataset_recorder-action-recover = Recover
dataset_recorder-action-refresh = Refresh
dataset_recorder-action-open_folder = Open Datasets Folder
dataset_recorder-action-reveal = Show in Folder
dataset_recorder-action-validate = Validate
dataset_recorder-action-delete = Delete
dataset_recorder-action-quarantine = Quarantine
dataset_recorder-action-confirm_delete = Are you sure you want to delete this dataset?
dataset_recorder-consent-label = I consent to local dataset recording for AI drift training without uploading my telemetry
dataset_recorder-hash-identifiers-label = Include one-session hashes of hardware identifiers
dataset_recorder-consent-required = Explicit consent is required to start recording
dataset_recorder-pseudonym-label = Subject Pseudonym (Optional)
dataset_recorder-pseudonym-placeholder = e.g. subject-01
dataset_recorder-profile-label = Recording Profile
dataset_recorder-profile-custom = Diagnostic recording / Minimum (Enabled)
dataset_recorder-profile-production = Production (Locked: Pending Dataset Gate)
dataset_recorder-metrics-time = Session Time
dataset_recorder-metrics-sampled_frames = Sampled Frames
dataset_recorder-metrics-written_frames = Written Frames
dataset_recorder-metrics-dropped_frames = Dropped Frames
dataset_recorder-metrics-queue_depth = Queue / Peak
dataset_recorder-metrics-resets = Captured Resets
dataset_recorder-metrics-trackers = Active Trackers
dataset_recorder-metrics-size = Disk Written
dataset_recorder-metrics-disk_free = Free Space
dataset_recorder-metrics-disk_used = Disk Used / Total
dataset_recorder-metrics-roster = Active Roster
dataset_recorder-metrics-finalizing_progress = Progress
dataset_recorder-readiness-ready = Ready to record
dataset_recorder-readiness-warnings = Hardware Warnings
dataset_recorder-readiness-errors = Blocking Readiness Issues
dataset_recorder-finding-HMD_UNAVAILABLE = Headset reference is absent or not sending tracking data
dataset_recorder-finding-NO_ASSIGNED_IMUS = At least one assigned physical IMU tracker is required
dataset_recorder-finding-LOW_DISK_SPACE = Insufficient free disk space (at least 250 MB required)
dataset_recorder-finding-STORAGE_ERROR = Dataset storage directory is not accessible
dataset_recorder-finding-UNKNOWN_HARDWARE = Tracker(s) have uncalibrated or unknown hardware metadata
dataset_recorder-finding-UNSUPPORTED_LAYOUT = Tracker layout does not meet recording requirements
dataset_recorder-finding-RECORDING_ALREADY_ACTIVE = Recording is already in progress
dataset_recorder-finding-SESSION_NOT_FOUND = Recording session not found
dataset_recorder-sessions-title = Recorded Sessions
dataset_recorder-sessions-empty = No recorded datasets found yet. Click "Start Recording" to capture a session.
dataset_recorder-sessions-valid = Valid
dataset_recorder-sessions-invalid = Invalid
dataset_recorder-recoverable-title = Recoverable Sessions Found
dataset_recorder-recoverable-description = Interrupted or unfinalized sessions were detected. You can quarantine, recover, or clean them up.
dataset_recorder-recoverable-quarantine = Move to Quarantine
dataset_recorder-validation-findings = Validation findings
dataset_recorder-severity-fatal = Fatal
dataset_recorder-severity-warning = Warning
dataset_recorder-severity-info = Ready
dataset_recorder-finding-DATASET_READY = This build passed the dataset-ready gate.
dataset_recorder-finding-DATASET_READY_GATE_PENDING = Production recording is locked until the dataset-ready report passes.
dataset_recorder-profile-production-ready = Dataset-ready evidence passed; production recording is available
dataset_recorder-profile-production-locked = Production profile is locked pending dataset-ready evidence
dataset_recorder-severity-error = Error
dataset_recorder-session-id = Session ID
dataset_recorder-action-confirm_cancel_button = Confirm cancellation
dataset_recorder-action-dismiss_cancel_button = Keep recording
dataset_recorder-action-confirm_delete_button = Confirm deletion
dataset_recorder-action-dismiss_delete_button = Keep dataset
dataset_recorder-unit-frames = frames
dataset_recorder-unit-trackers = trackers

# AI drift model control plane
ai_drift-title = AI drift correction
ai_drift-description = Server-controlled neural correction for IMU tracker drift in real time
ai_drift-tabs-label = AI drift work areas
ai_drift-tab-correction = Correction
ai_drift-tab-datasets = Datasets
ai_drift-tab-personal = Personal Training
ai_drift-personal-title = Personal Training
ai_drift-personal-pending = The dedicated workspace is ready. Trainer installation, profiles, eligibility, and jobs become available with the personal-training control plane.
ai_drift-fail_open_notice = Standard tracking remains active if AI correction or its runtime becomes unavailable.
ai_drift-updater-title = Automatic updates (GitHub)
ai_drift-updater-description = Synchronization and update checks from the main branch
ai_model-title = Model control plane
ai_model-description = Import, verify, activate, configure, and monitor the server ONNX model
ai_model-runtime-health = Runtime health
ai_model-runtime-load_state = Model state
ai_model-runtime-provider = Probe-verified active provider
ai_model-runtime-latency = Inference latency
ai_model-runtime-package = Server runtime package
ai_model-runtime-rate = Inference rate
ai_model-runtime-queue = Queue / drops
ai_model-runtime-active_model = Active model acknowledged by server
ai_model-runtime-no_active_model = No active model
ai_model-runtime-no_active_provider = No provider has passed activation probe
ai_model-unit-ms = ms
ai_model-health-disconnected = Server disconnected
ai_model-health-disabled = Disabled
ai_model-health-unavailable = Runtime unavailable
ai_model-health-healthy = Healthy
ai_model-health-degraded = Degraded
ai_model-health-watchdog = Stopped by watchdog
ai_model-load-unavailable = Runtime unavailable
ai_model-load-unloaded = Unloaded
ai_model-load-loading = Loading
ai_model-load-active = Active
ai_model-load-error = Load error
ai_model-action-enable = Enable correction
ai_model-action-enable-locked = Enable locked: readiness required
ai_model-action-disable-intent = Disable saved correction intent
ai_model-action-load-diagnostic = Load for diagnostics
ai_model-history-switch-diagnostic = Load diagnostic model
ai_model-readiness-ready = Production inference ready.
ai_model-readiness-locked = Production correction is locked.
ai_model-readiness-evidence-model = Evidence model SHA-256
ai_model-readiness-persisted-intent = Saved correction intent
ai_model-readiness-effective-state = Effective runtime correction
ai_model-state-on = on
ai_model-state-off = off
ai_model-action-disable = Disable correction
ai_model-action-unload = Unload model
ai_model-action-refresh_catalog = Refresh catalog
ai_model-action-download = Download and verify
ai_model-action-activate = Activate selected model
ai_model-action-browse = Browse…
ai_model-action-import = Import and verify
ai_model-action-apply_settings = Apply settings
ai_model-action-rollback = Roll back to previous model
ai_model-history-title = Recent and pinned models
ai_model-history-refresh = Refresh
ai_model-history-empty = No server model history yet. Activating a verified model adds it here.
ai_model-history-personal = Personal
ai_model-history-active = Active
ai_model-history-compatible = Compatible
ai_model-history-pin = Pin model
ai_model-history-unpin = Unpin model
ai_model-history-switch = Switch
ai_model-catalog-title = Verified model catalog
ai_model-catalog-label = Model
ai_model-catalog-empty = No server models available
ai_model-catalog-downloaded = downloaded
ai_model-catalog-remote = remote
ai_model-catalog-incompatible = This model is incompatible with the server configuration
ai_model-import-title = Local model import
ai_model-import-model = ONNX model file
ai_model-import-metadata = Sidecar metadata (JSON)
ai_model-import-electron_only = Local import is available in the desktop application because the server requires user-approved file paths.
ai_model-settings-title = Inference and safety settings
ai_model-settings-provider = Requested provider
ai_model-settings-provider_notice = This is a request, not hardware detection. A provider becomes active only after the server successfully runs its model probe.
ai_model-settings-context = Context frames
ai_model-settings-confidence = Confidence threshold (0–1)
ai_model-settings-max_correction = Maximum correction (rad)
ai_model-settings-max_rate = Maximum angular rate (rad/s)
ai_model-settings-max_acceleration = Maximum angular acceleration (rad/s²)
ai_model-settings-smoothing = Smoothing (0–1)
ai_model-settings-stale_decay = Stale correction decay (s)
ai_model-settings-legacy_mode = Legacy drift mode
ai_model-settings-legacy_replace = Replace legacy correction
ai_model-settings-legacy_compose = Compose with legacy correction
ai_model-mapping-title = Tracker, body-role, and model-slot mapping
ai_model-mapping-empty = No mappings configured. Add one mapping for every tracker handled by the model.
ai_model-mapping-tracker = Tracker ID
ai_model-mapping-role = Body role ID
ai_model-mapping-slot = Model slot
ai_model-mapping-add = Add mapping
ai_model-mapping-remove = Remove mapping
ai_model-mapping-apply = Apply mappings
ai_model-trackers-title = Server-reported tracker correction state
ai_model-trackers-confidence = Confidence
ai_model-trackers-correction = Correction
ai_model-trackers-reason = Status detail
ai_model-trackers-applied = Applied
ai_model-trackers-not_applied = Not applied
ai_model-progress-idle = Waiting
ai_model-progress-catalog = Fetching catalog…
ai_model-progress-model = Downloading model…
ai_model-progress-metadata = Downloading metadata…
ai_model-progress-verifying = Verifying hashes and metadata…
ai_model-progress-importing = Importing into managed storage…
ai_model-progress-activating = Running the provider probe and activating…
ai_model-progress-completed = Operation completed
ai_model-progress-failed = Operation failed
ai_model-error-title = Model operation failed
ai_model-error-not_connected = The server is not connected.
ai_model-error-timeout = The server did not acknowledge the model operation in time.
ai_model-error-disconnected = The server disconnected during the model operation.
